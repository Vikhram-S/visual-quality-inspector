...
REAL_HOLDOUT_SOURCE_DIR = os.path.join(DATASET_DIR, "real_holdout_source")
...
REAL_PHOTO_URLS = [
    ...
    "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=500", # Landscape Forest
...
def create_synthetic_clean_image(width=512, height=512, seed=42):
    ...
    if pattern_type == 0:
        ...
        for _ in range(20):
            ...
            cv2.line(img, (x1, y1), (x2, y2), (40, 210, 240), 3)
...
def acquire_real_clean_photos(count=30):
    """
    Acquires real clean photos for the real-world holdout evaluation set.
    1. First checks committed local source images in `ml/dataset/real_holdout_source/`.
    2. Only downloads from Unsplash if local source files are missing.
    3. Logs a visible WARNING if a download fails, substituting a synthetic fallback.
    Returns: (list of image arrays, genuine_count, fallback_count)
    """
    os.makedirs(REAL_HOLDOUT_SOURCE_DIR, exist_ok=True)
    real_images = []
    genuine_count = 0
    fallback_count = 0
    ...
    for idx in range(count):
        local_path = os.path.join(REAL_HOLDOUT_SOURCE_DIR, f"real_base_{idx:04d}.jpg")
        img_arr = None

        # (a) Check local committed source image first
        if os.path.exists(local_path):
            img_arr = cv2.imread(local_path)
            if img_arr is not None and img_arr.size > 0:
                genuine_count += 1

        # (b) Attempt live download if local image missing
        if img_arr is None:
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=5) as resp:
                    ...
                    if img is not None and img.size > 0:
                        ...
                        cv2.imwrite(local_path, img_arr)
                        genuine_count += 1
            except Exception as e:
                img_arr = None

        # (c) Visible warning on synthetic fallback
        if img_arr is None:
            fallback_count += 1
            print(f"WARNING: Unsplash download failed for image index {idx}. Substituting synthetic procedural fallback.")
            img_arr = create_synthetic_clean_image(512, 512, seed=8000 + idx)

        real_images.append(img_arr)

    print(f"\nReal holdout set provenance: {genuine_count}/{count} genuine photos, {fallback_count}/{count} synthetic fallback")
    return real_images, genuine_count, fallback_count
...
def generate_dataset(num_train_base=100, num_test_base=30, num_real_holdout=30):
    ...
    for d in [TRAIN_DIR, TEST_DIR, REAL_HOLDOUT_DIR, REAL_HOLDOUT_SOURCE_DIR]:
    ...
    real_clean_photos, genuine_cnt, fallback_cnt = acquire_real_clean_photos(num_real_holdout)
    for i, real_img in enumerate(real_clean_photos):
        save_split_samples(REAL_HOLDOUT_DIR, real_img, i, is_real=True)

    # Save dataset provenance metadata
    prov_data = {
        "num_real_holdout_base": num_real_holdout,
        "genuine_photos_count": genuine_cnt,
        "synthetic_fallback_count": fallback_cnt,
        "provenance_summary": f"{genuine_cnt}/{num_real_holdout} genuine photos, {fallback_cnt}/{num_real_holdout} synthetic fallback"
    }
    with open(os.path.join(REAL_HOLDOUT_DIR, "provenance.json"), "w") as f:
    ...
    print(f"  - Real Source Cache DIR:  {REAL_HOLDOUT_SOURCE_DIR}")
    print(f"  - Provenance Summary:     {prov_data['provenance_summary']}")
